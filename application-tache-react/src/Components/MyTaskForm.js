    ...
    return (
        <div className={"flex flex-grow flex-col h-screen items-center bg-gradient-to-r from-blue-300 to-gray-500"}>
            ...
            <form className={"flex flex-col w-1/2 bg-white p-4 rounded-lg shadow-lg mt-4"}>
                ...
                <input type={"text"} className={"border-2 p-2 rounded-lg"} value={task.nom} onChange={(e) => setTask({...task, nom: e.target.value})}/>
                {errors.title && <p className={"text-red-500"}>{errors.title}</p>}
                <label className={"text-lg"}>Description</label>
                <input type={"text"} className={"border-2 p-2 rounded-lg"} value={task.description} onChange={(e) => setTask({...task, description: e.target.value})}/>
                {errors.description && <p className={"text-red-500"}>{errors.description}</p>}
                <label className={"text-lg"}>Date de début</label>
                <input type={"datetime-local"} className={"border-2 p-2 rounded-lg"} value={task.dateDebut} onChange={(e) => setTask({...task, dateDebut: e.target.value})}/>
                {errors.start && <p className={"text-red-500"}>{errors.start}</p>}
                <label className={"text-lg"}>Date de fin</label>
                <input type={"datetime-local"} className={"border-2 p-2 rounded-lg"} value={task.dateFin} onChange={(e) => setTask({...task, dateFin: e.target.value})}/>
                {errors.end && <p className={"text-red-500"}>{errors.end}</p>}
                <label className={"text-lg"}>Couleur</label>
                <input type={"color"} className={"border-2 p-2 rounded-lg"} value={task.couleur} onChange={(e) => setTask({...task, couleur: e.target.value})}/>
                <button type={"button"} className={"bg-blue-500 text-white p-2 rounded-lg"} onClick={saveTask}>Ajouter</button>
            </form>
        </div>
    )
}
